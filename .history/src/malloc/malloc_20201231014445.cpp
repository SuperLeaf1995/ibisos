...
void *malloc(size_t size)
{
    ...
    struct memory_block *block = (memory_block*) ((unsigned int)size + block_count + sizeof(struct memory_block));
    //Reduce free space
    block->free_space = MAX_ALLOC_MEMORY - size;
    //Set the size
    block->size = size;
    //Create an item given size and block count
    void *item = (void*) ((unsigned int) size + block_count + sizeof(void*));
    //Set the item
    block->item = item;
